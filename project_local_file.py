import time
import multiprocessing
import socket
import threading
import json
from shifter import Shifter
import RPi.GPIO as GPIO
import os
from urllib.parse import parse_qs
import math

# --- Turret heights in cm ---
turret_height_self = 3.0
turret_height_other = 0.0

# --- GPIO Setup ---
GPIO.setmode(GPIO.BCM)
laser = 22
GPIO.setup(laser, GPIO.OUT)
GPIO.output(laser, GPIO.LOW)

# --- Shared memory for stepper shifter ---
myArray = multiprocessing.Array('i', 2)

# --- Load positions ---
positions = {}
def load_positions():
    global positions
    script_dir = os.path.dirname(os.path.realpath(__file__))
    filename = os.path.join(script_dir, "test_positions.json")
    if os.path.exists(filename):
        try:
            with open(filename, "r") as f:
                positions = json.load(f)
            print("Loaded JSON position file successfully:")
            print(json.dumps(positions, indent=2))
        except Exception as e:
            print("Error loading JSON:", e)
    else:
        print("JSON file not found:", filename)

load_positions()

# --- Calibration / zero storage ---
zero_angles = {"m1": 0.0, "m2": 0.0}   # saved zero positions (physical angles)
calibration_offsets = {"m1": 0.0, "m2": 0.0}  # manual offsets
self_team = {"id": None}
current_target = {"team": None}

# --- Stepper class ---
class Stepper:
    seq = [0b0001,0b0011,0b0010,0b0110,0b0100,0b1100,0b1000,0b1001]
    delay = 500
    steps_per_degree = 4*1024/360

    def __init__(self, shifter, lock, index):
        self.s = shifter
        self.lock = lock
        self.index = index
        self.angle = 0
        self.step_state = 0
        self.shifter_bit_start = 4*index

    def _sgn(self, x):
        return 0 if x==0 else int(abs(x)/x)

    def _step(self, direction):
        with self.lock:
            self.step_state = (self.step_state + direction) % 8
            myArray[self.index] &= ~(0b1111 << self.shifter_bit_start)
            myArray[self.index] |= (Stepper.seq[self.step_state] << self.shifter_bit_start)
            final = 0
            for val in myArray:
                final |= val
            self.s.shiftByte(final)
        self.angle = (self.angle + direction/Stepper.steps_per_degree) % 360
        time.sleep(Stepper.delay/1e6)

    def _rotate(self, delta):
        direction = self._sgn(delta)
        steps = int(abs(delta)*Stepper.steps_per_degree)
        for _ in range(steps):
            self._step(direction)

    def rotate(self, delta):
        p = multiprocessing.Process(target=self._rotate, args=(delta,))
        p.start()
        return p

    def goAngle(self, target):
        delta = (target - self.angle + 540) % 360 - 180
        return self.rotate(delta)

    def zero(self):
        self.angle = 0

# --- Laser ---
def test_laser():
    GPIO.output(laser, GPIO.HIGH)
    time.sleep(1)
    GPIO.output(laser, GPIO.LOW)

# --- Zero & Calibration ---
def save_zero(m1, m2):
    zero_angles["m1"] = m1.angle
    zero_angles["m2"] = m2.angle
    print(f"Saved zero angles: {zero_angles}")

def return_to_zero(m1, m2):
    target_el = zero_angles["m1"]
    target_az = zero_angles["m2"]
    print(f"Returning to zero... targets: el={target_el:.3f}, az={target_az:.3f}")
    p1 = m1.goAngle(target_el)
    p2 = m2.goAngle(target_az)
    p1.join()
    p2.join()

# --- Aim at another team ---
def aim_at_team(m1, m2, target_team):
    if self_team["id"] is None:
        print("ERROR: Self team not set")
        return
    if target_team not in positions.get("turrets", {}):
        print("Target team not found:", target_team)
        return
    st = self_team["id"]
    if st not in positions["turrets"]:
        print("ERROR: self team not in positions")
        return

    current_target["team"] = target_team

    # Positions and radius
    r_self = positions["turrets"][st]["r"]
    r_tgt  = positions["turrets"][target_team]["r"]
    th_self = positions["turrets"][st]["theta"]
    th_tgt  = positions["turrets"][target_team]["theta"]

    x_self = r_self*math.cos(th_self)
    y_self = r_self*math.sin(th_self)
    x_tgt = r_tgt*math.cos(th_tgt)
    y_tgt = r_tgt*math.sin(th_tgt)

    z_self = turret_height_self
    z_tgt = turret_height_other

    dx = x_tgt - x_self
    dy = y_tgt - y_self
    dz = z_tgt - z_self

    # Horizontal distance and azimuth
    horiz_dist = math.hypot(dx, dy)
    az_rad = math.atan2(dy, dx) - th_self
    az_deg = -az_rad * 180.0 / math.pi
    az_deg = (az_deg + 180) % 360 - 180
    az_target = (zero_angles["m2"] + az_deg + calibration_offsets["m2"]) % 360

    # Elevation
    el_rad = math.atan2(dz, horiz_dist)
    el_deg = -el_rad * 180.0 / math.pi
    el_target = (zero_angles["m1"] + el_deg + calibration_offsets["m1"]) % 360

    print(f"Aiming at team {target_team}: az={az_target:.2f}, el={el_target:.2f} | horiz_dist={horiz_dist:.2f}, dz={dz:.2f}")

    p1 = m1.goAngle(el_target)
    p2 = m2.goAngle(az_target)
    p1.join()
    p2.join()

# --- Parse POST ---
def parsePOSTdata(data):
    idx = data.find("\r\n\r\n")
    if idx==-1: return {}
    post = data[idx+4:]
    parsed = parse_qs(post, keep_blank_values=True)
    return {k:v[0] for k,v in parsed.items()}

# --- Web page ---
def web_page(m1_angle, m2_angle):
    def jog_buttons(name):
        buttons = [-90, -45, -15, -5, -1, -0.5, 0.5, 1, 5, 15, 45, 90]
        html_buttons = ""
        for b in buttons:
            html_buttons += f'<button name="{name}_jog" value="{b}">{b:+}°</button> '
        return html_buttons

    current_team_str = self_team["id"] or "None"
    target_team_str = current_target["team"] or "None"

    html = f"""
    <html>
    <head><title>Laser Turret</title></head>
    <body style="font-family: Arial; text-align:center; margin-top:40px;">
        <h2>Laser Turret Control</h2>
        <p>Current team: {current_team_str} | Aiming at: {target_team_str}</p>
        <form action="/" method="POST">
            <h3>Set Turret Team</h3>
            <input type="text" name="self_team" placeholder="Your team number"><br>
            <input type="submit" name="set_self_team" value="Set"><br>

            <h3>Aim at Another Team</h3>
            <input type="text" name="team_box" placeholder="Target team #"><br>
            <input type="submit" name="aim_team" value="Aim"><br>

            <h3>Manual Motor Control</h3>
            <h4>Azimuth</h4>
            <input type="text" name="m2" value="{m2_angle}"><br>
            {jog_buttons('m2')}

            <h4>Elevation</h4>
            <input type="text" name="m1" value="{m1_angle}"><br>
            {jog_buttons('m1')}

            <br><input type="submit" value="Rotate Motors"><br>

            <h3>Calibration</h3>
            <input type="submit" name="return_zero" value="Return to Zero"><br>
            <input type="submit" name="save_zero" value="Save Zero"><br>

            <h3>Laser</h3>
            <input type="submit" name="laser" value="Test Laser (1s)"><br>
        </form>
    </body>
    </html>
    """
    return bytes(html, "utf-8")

# --- Web server ---
def serve_web(m1, m2):
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR,1)
    s.bind(('',8080))
    s.listen(3)
    print("running at IP:8080")

    while True:
        conn, addr = s.accept()
        try:
            msg = conn.recv(4096).decode(errors='ignore')
        except: 
            conn.close()
            continue

        if msg.startswith("POST"):
            data = parsePOSTdata(msg)

            # Set team
            if "set_self_team" in data and data.get("self_team","").strip()!="":
                self_team["id"] = data["self_team"].strip()
                print("Self team set:", self_team["id"])

            # Aim team
            if "aim_team" in data and data.get("team_box","").strip()!="":
                aim_at_team(m1, m2, data["team_box"].strip())

            # Manual control
            if "m1" in data and data["m1"].strip()!="":
                try: p = m1.goAngle(float(data["m1"])); p.join()
                except: pass
            if "m2" in data and data["m2"].strip()!="":
                try: p = m2.goAngle(float(data["m2"])); p.join()
                except: pass

            # Jog buttons
            if "m1_jog" in data:
                try: p = m1.rotate(float(data["m1_jog"])); p.join()
                except: pass
            if "m2_jog" in data:
                try: p = m2.rotate(float(data["m2_jog"])); p.join()
                except: pass

            # Calibration
            if "save_zero" in data: save_zero(m1,m2)
            if "return_zero" in data: return_to_zero(m1,m2)

            # Laser
            if "laser" in data: test_laser()

        try:
            response = web_page(m1.angle, m2.angle)
            conn.send(b"HTTP/1.1 200 OK\r\nContent-Type: text/html; charset=utf-8\r\n\r\n")
            conn.sendall(response)
        except: pass
        conn.close()

# --- Main ---
if __name__=="__main__":
    s = Shifter(23,24,25)
    lock1 = multiprocessing.Lock()
    lock2 = multiprocessing.Lock()

    m1 = Stepper(s, lock1, 0)  # elevation
    m2 = Stepper(s, lock2, 1)  # azimuth
    m1.zero()
    m2.zero()

    t = threading.Thread(target=serve_web, args=(m1,m2), daemon=True)
    t.start()

    try:
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        print("Exiting.")
